...
class AddPointDialogFragment : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val builder = AlertDialog.Builder(requireActivity(), R.style.PopupTheme).setView(view)
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val processedFile = if (mosaicBitmap != null) {
                    // 얼굴이 탐지된 경우: 모자이크 처리된 이미지 사용
                    Log.d("AddPointDialog", "✅ 얼굴 탐지 및 모자이크 처리 성공!")
                    ...
                    saveBitmapToFile(mosaicBitmap, "mosaic_")
                } else {
                    // 얼굴이 탐지되지 않은 경우: 원본 이미지 그대로 사용
                    Log.d("AddPointDialog", "ℹ️ 얼굴이 탐지되지 않음 - 원본 이미지 사용")
                    capturedImageFile // 원본 파일 그대로 사용
                }

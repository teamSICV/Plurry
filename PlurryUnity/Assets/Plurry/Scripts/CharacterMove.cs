...
public class CharacterMove : MonoBehaviour
{
    ...
    private string currentEnterTrigger = "";
    ...
    void Update()
    {
        ...
        if(!bisCanPlayerInput)
        {
            ...
            if (Input.touchCount == 1)
            {
                LogLS.Log("EndPlayerState Touch Begin");
    ...
    private void TouchRay(Vector2 position)
    {
        ...
        if (currentEnterTrigger!="")
        {
            //LogLS.Log("currentEnterTrigger Ray");
            int triggerLayer = LayerMask.GetMask("Trigger");
            if (Physics.Raycast(ray, out hit, Mathf.Infinity, triggerLayer))
            {
                //LogLS.Log("hit : " + hit.transform.tag);
                //LogLS.Log("currentEnterTrigger : " + currentEnterTrigger);
                if (hit.transform.tag == currentEnterTrigger)
                {
                    GetComponent<PlayerState>().SendMessage("SetPlayerState", currentEnterTrigger);
                    string functionName = "Unity" + currentEnterTrigger + "TriggerEnter";
                    GameObject.FindWithTag("GameController").GetComponent<GameController>().SendMessage("SendCommendToAndroid", functionName);
                    return;
                }
            }
        }
    ...
    void SetcurrentEnterTrigger(string inTriggerString)
    {
        //LogLS.Log(inTriggerString);
        currentEnterTrigger = inTriggerString;
    }
}
